...
export default function BridgeSection({
  ...
  const variants = {
    default: {
      ...
      button: 'bg-[#eab308] hover:bg-yellow-500 text-[#0a1628]',
    },
    light: {
      container: 'bg-[#e8f0fe] text-[#0a1628]',
      title: 'text-[#0a1628]',
      subtitle: 'text-gray-600',
      button: 'bg-[#0a1628] hover:bg-[#1a2a4a] text-white',
    },
    dark: {
    ...
    compact: {
      ...
      button: 'bg-[#eab308] hover:bg-yellow-500 text-[#0a1628] text-sm px-4 py-1.5',
    },
  };
    ...
    <section className={`w-full py-8 md:py-12 ${style.container} ${className}`}>
      <div className="container mx-auto px-4 md:px-6">
        <div className={`max-w-4xl mx-auto text-center ${isRTL ? 'rtl' : 'ltr'}`}>
          ...
          <Link
            href={`/${locale}${ctaLink}`} // ✅ Ajout du préfixe de locale
            className={`inline-block px-6 md:px-8 py-2.5 md:py-3 rounded-md font-bold transition transform hover:scale-105 ${style.button}`}
          >
