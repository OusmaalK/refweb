  ...
  return (
    <section ref={sectionRef} className="py-12 md:py-16 bg-[#f8fafc]">
      <div className="container mx-auto px-4 md:px-6">
        ...
        {/* ✅ Grille : 2 colonnes sur mobile, 4 colonnes sur desktop */}
        <div className={`grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 ${isRTL ? 'rtl' : 'ltr'}`}>
          {STEPS.map((step, index) => (
            <div key={index} className="flex flex-col items-center text-center group relative">
              <div className="relative w-20 h-20 md:w-28 md:h-28 rounded-full border-2 border-[#0a1628] flex items-center justify-center mb-3 group-hover:border-[#eab308] transition">
                <step.icon className="w-6 h-6 md:w-10 md:h-10 text-[#0a1628] group-hover:text-[#eab308] transition" />
              </div>
              {/* ✅ Flèche uniquement sur desktop */}
              {index < STEPS.length - 1 && (
                <ArrowRight className="hidden md:block absolute -right-4 top-1/2 -translate-y-1/2 text-[#0a1628]" />
              )}
